...
class ImageHandler(BaseHTTPRequestHandler):
    ...
    def _handle_edit(self):
        ...
        try:
            ...
            # diffusers img2img
                ...
                self._json(500, {"error": {"message": "Model does not support editing"}})
                return
    ...
    def _handle_sdapi_txt2img(self):
        ...
        try:
            ...
            print(f"  [sdapi] done in {elapsed:.1f}s ({len(img_b64)//1024}KB)", flush=True)
            self._json(200, {
                "images": [f"data:image/png;base64,{img_b64}"],
                "parameters": body,
                "info": json.dumps({"seed": seed, "prompt": prompt, "time": f"{elapsed:.1f}s"}),
            })
        except Exception as e:
    ...
    def do_GET(self):
        if self.path in ("/health", "/"):
            ...
            self._json(200, [{"title": _model_name or "image-gen", "model_name": _model_name or "image-gen"}])
        elif self.path == "/sdapi/v1/options":
            self._json(200, {"sd_model_checkpoint": _model_name or "image-gen"})
