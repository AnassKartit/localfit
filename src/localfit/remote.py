...
def _generate_notebook_image(
    ...
    """Generate a Kaggle notebook that serves an image generation model via diffusers + Cloudflare.

    Same pattern as LLM notebook: install deps → load model → tunnel → ntfy → keep alive.
    Uses localfit's image_server.py OpenAI-compatible API.
    """
    # Map friendly names to HuggingFace repos for diffusers
    # Prefer non-gated models (FLUX.2 Klein, Z-Image) for Kaggle where HF token may not be set
    model_repos = {
    ...
    hf_repo = model_repos.get(model_name, model_repos["flux2-klein-4b"])

    script = f'''#!/usr/bin/env python3
"""LocalFit remote image serve — auto-generated for Kaggle.

Model: {model_name} ({hf_repo})
GPU: Kaggle {kaggle_gpu["name"]} ({kaggle_gpu["vram_gb"]}GB VRAM)
API: OpenAI-compatible /v1/images/generations
Generated by: localfit serve {model_name} --remote kaggle --image
"""
import subprocess, sys, time, os, re, json, base64, io, threading
from http.server import HTTPServer, BaseHTTPRequestHandler

# ── 1. Install deps ──
print("LOCALFIT_STATUS=installing"); sys.stdout.flush()
os.system("pip install -q -U git+https://github.com/huggingface/diffusers.git transformers accelerate sentencepiece protobuf 2>&1 | tail -3")
os.system("wget -q https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64.deb && dpkg -i cloudflared-linux-amd64.deb")

# ── 2. Detect GPU ──
gpu_check = subprocess.run(["nvidia-smi", "--query-gpu=name,memory.total",
                            "--format=csv,noheader"], capture_output=True, text=True)
gpu_name = gpu_check.stdout.strip() if gpu_check.returncode == 0 else "none"
print(f"LOCALFIT_GPU={{gpu_name}}"); sys.stdout.flush()

# ── 3. Load model — AutoPipeline handles ANY model automatically ──
print("LOCALFIT_STATUS=loading_{model_name}"); sys.stdout.flush()
import torch
from diffusers import DiffusionPipeline

device = "cuda" if torch.cuda.is_available() else "cpu"
print(f"LOCALFIT_DEVICE={{device}}")
sys.stdout.flush()
...
try:
    pipe = DiffusionPipeline.from_pretrained(
        "{hf_repo}", torch_dtype=torch.bfloat16, token=hf_token
    )
    pipe.enable_model_cpu_offload()
    print(f"LOCALFIT_STATUS=model_loaded")
except Exception as e:
    if "401" in str(e) or "403" in str(e) or "gated" in str(e).lower():
        print(f"LOCALFIT_ERROR=gated_model")
        print(f"This model requires a HuggingFace token. Set HF_TOKEN as a Kaggle secret.")
        print(f"Accept the license at https://huggingface.co/{hf_repo} first.")
    else:
        print(f"LOCALFIT_ERROR=load_failed: {{e}}")
    sys.stdout.flush(); sys.exit(1)
sys.stdout.flush()

# ── 4. Serve OpenAI-compatible image API ──
class ImageHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        if self.path == "/v1/images/generations":
            length = int(self.headers.get("Content-Length", 0))
            body = json.loads(self.rfile.read(length)) if length else {{}}
            prompt = body.get("prompt", "a landscape")
            size = body.get("size", "512x512")
            steps = body.get("steps", body.get("sample_steps", 4))
            w, h = (int(x) for x in size.split("x")) if "x" in size else (512, 512)
            try:
                result = pipe(prompt=prompt, width=w, height=h, num_inference_steps=steps)
                pil_img = result.images[0]
                buf = io.BytesIO()
                pil_img.save(buf, format="PNG")
                self._json(200, {{"created": int(time.time()), "data": [{{"b64_json": base64.b64encode(buf.getvalue()).decode()}}]}})
            except Exception as e:
...
print(f"LOCALFIT_STATUS=api_ready_port_{port}"); sys.stdout.flush()
...
if endpoint:
    ...
    try:
        import urllib.request as _ur
        _ntfy_topic = "localfit-img-" + "{model_name}".replace("/", "-").replace(":", "-").replace(" ", "-")[:40]
        _ur.urlopen(f"https://ntfy.sh/{{_ntfy_topic}}", data=endpoint.encode(), timeout=5)
        print(f"LOCALFIT_NTFY_SENT={{_ntfy_topic}}")
    except Exception as _ntfy_err:
        print(f"LOCALFIT_NTFY_FAILED={{_ntfy_err}}")
    ...
    print(f"")
    print(f"========================================")
    print(f"  Image API ready at: {{endpoint}}")
    print(f"========================================")
    print("")
    print("Test:")
    print(f"  curl {{endpoint}}/v1/images/generations -H 'Content-Type: application/json' -d '" + '{{"prompt": "a cyberpunk city at sunset", "size": "512x512", "steps": 4}}' + "'")
    print("")
    print("Open WebUI: Settings → Images → OpenAI URL → " + endpoint)
    sys.stdout.flush()
else:
    print("LOCALFIT_ERROR=tunnel_failed")
    sys.stdout.flush()
